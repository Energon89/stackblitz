...
export class DrawingToolInterpretor {
  constructor(commandLine) {
    this.commands = [];
    this.validateCommands(commandLine);
    this.drawingTool = new DrawingTool(
      this.canvasCommand.width,
      this.canvasCommand.height
    );
  }
  draw() {
    ...
    for (let i = 0; i < this.commands.length; i++) {
      ...
      if (command.type == "L") {
        ...
        this.drawingTool.bufferRectangle(
          command.x1,
          command.y1,
          command.x2,
          command.y2,
  ...
  validateCommands(commandLine) {
    ...
    commands = commands.filter(function(n) {
      return n;
    }); //clean empty values
  ...
  validateCanvasCommand(command) {
    ...
    if (command[0] !== "C") {
      throw new Error(
        'The first command must be the canvas parameters, i.e. "C {width} {height}"'
      );
    }
    this.canvasCommand = {
