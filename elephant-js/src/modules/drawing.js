export class DrawingTool {
  ...
  bufferLine(x1, y1, x2, y2, character, canvas) {
    ...
    if (x1 == x2) {
      ...
      throw new Error(
        "The coordinates do not form a straight line. Must be either {x1} equals {x2} or {y1} equals {y2}."
      );
    }
  }
    ...
    if (x < 1 || y < 1 || x >= this.canvasWidth || y >= this.canvasHeight) {
      throw new Error(
        "x: " +
          ...
          " and y: " +
          y +
          " is an invalid coordinate. The coordinates must be pointed inside the canvas. width: " +
          this.canvasWidth +
          ", height: " +
          this.canvasHeight
      );
